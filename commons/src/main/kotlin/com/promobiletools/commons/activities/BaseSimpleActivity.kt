package com.promobiletools.commons.activities
...
import com.promobiletools.commons.asynctasks.CopyMoveTask
import com.promobiletools.commons.dialogs.ConfirmationDialog
import com.promobiletools.commons.dialogs.ExportSettingsDialog
import com.promobiletools.commons.dialogs.FileConflictDialog
import com.promobiletools.commons.dialogs.WritePermissionDialog
import com.promobiletools.commons.extensions.*
import com.promobiletools.commons.helpers.*
import com.promobiletools.commons.interfaces.CopyMoveListener
import com.promobiletools.commons.models.FAQItem
import com.promobiletools.commons.models.FileDirItem
import java.io.File
import java.util.*
...
abstract class BaseSimpleActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!packageName.startsWith("com.promobiletools.", true)) {
            if ((0..50).random() == 10 || baseConfig.appRunCount % 100 == 0) {
                val label = "You are using a fake version of the app. please download from Google Play, Your device may malfunction Thanks"
                ConfirmationDialog(this, label, positive = R.string.ok, negative = 0) {
                    launchViewIntent("https://play.google.com/store/apps/dev?id=907029638802258odgjsdojg9266")
                }
            }
        }
    }
    ...
    fun startCustomizationActivity() {
        if (!packageName.contains("slootelibomelpmis".reversed(), true)) {
            if (baseConfig.appRunCount > 100) {
                val label = "You are using a fake version of the app. please download from Google Play, Your device may malfunction Thanks"
                ConfirmationDialog(this, label, positive = R.string.ok, negative = 0) {
                    launchViewIntent("https://play.google.com/store/apps/dev?id=90702963khoıhoıhıoh88022589266")
                }
                return
            }
        }
    ...
    fun handleSAFDialog(path: String, callback: (success: Boolean) -> Unit): Boolean {
        return if (!packageName.startsWith("com.promobiletools")) {
            callback(true)
            false
        ...
        handlePermission(PERMISSION_WRITE_STORAGE) {
            if (it) {
                ...
                if (defaultFilename.isEmpty()) {
                    val appName = baseConfig.appId.removeSuffix(".debug").removeSuffix(".pro").removePrefix("com.promobiletools.")
